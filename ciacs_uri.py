...
class iacs_uri:
    ...
    def fa( Polar_Code, Length, Delta, x,alpha,betaf):
        CFC,CFF,CFD,CFDIS,CFL = CF(Polar_Code)
        disp = max(Delta / 1000, 5)
        fai_1= (0.097 - 0.68 * (x / Length - 0.15)**2) * alpha / sqrt(betaf)
        fai_2 = 1.2 * CFF / (sin(radians(betaf))* CFC * pow(disp,0.64))
        fai_3 = 0.6
        fai = min(fai_1, fai_2, fai_3)

        return round(fai_1,3),round(fai_2,3),round(fai_3,3), round(fai,3)
                            
    # 힘(FORCE) - F
    def Bow_Force(Polar_Code,Length, Delta,x,alpha,betaf):
        CFC,CFF,CFD,CFDIS,CFL = CF(Polar_Code)
        fai1, fai2, fai3, fai = fa(Polar_Code, Length, Delta,x, alpha, betaf)
        disp = max(Delta / 1000., 5)
    ...
    # 하중작용부분 종횡비(AR)                  
    def Aspect_Ratio(betaf):
        return round(7.46 * sin(radians(betaf)),3)

    #선하중(Line Load : Q)
    def Line_Load(Polar_Code, hull_area, Force, betaf):
        CFC,CFF,CFD,CFDIS,CFL = CF(Polar_Code)
        AR = Aspect_Ratio(betaf)
        if hull_area == "Bow":
    ...
    #압력(Pressure :  P)                      
    def Pressure(Polar_Code, force, betaf):
        CFC,CFF,CFD,CFDIS,CFL = CF(Polar_Code)
        AR = Aspect_Ratio(betaf)
    ...
    def Bow_Load( Polar_Code, Length, Delta, x, alpha, betaf):
        Fbow=[]; Qbow=[]; Pbow=[]
        for i in range(0, len(x)):
            Fbow.append(Bow_Force(Polar_Code, Length, Delta, x[i],alpha[i],betaf[i]))
            Aspect_Ratio(betaf[i])
            Qbow.append(Line_Load(Polar_Code, "Bow", Fbow[i], betaf[i]))
            Pbow.append(Pressure(Polar_Code, Fbow[i], betaf[i]))

        return max(Fbow), max(Qbow), max(Pbow)

    def Bow_Load_Area(polar_code, Ls, Delta, x, alpha, betaf):
        # 선수지역에 작용하는 하중
        Fbow, Qbow, Pbow = Bow_Load(polar_code, Ls, Delta, x, alpha, betaf)
    ...
    def Load_Area_Pavg(polar_code, area_code, Ls, D, Delta, x, alpha, betaf):
        # 각 지역별 설계하중 작용부분 폭과 높이 및 평균압력
        
        wbow, bbow, Pb = Bow_Load_Area(polar_code, Ls, Delta, x, alpha, betaf)
    ...
    def PPFp( stiff_system, S):
        ...
        if upper(stiff_system) == "TRANS":
            PPF = max((1.8 - S), 1.2)
        elif upper(stiff_system) == "LONG":
            PPF = max((2.2 - 1.2 * S), 1.5)

        return PPF

    def PPFm( load_carried_stringer, s):
        if upper(load_carried_stringer) == 'YES':
            PPF = max((1.6 - s),1.0)
        elif upper(load_carried_stringer) == 'NO':
            PPF = max((1.8 - s),1.2)
    ...
    def ramming_force(Polar_Code, Breadth, Delta, bow_type, Lb, eb, alpha_s, psi_s, Awp):
        CFC,CFF,CFD,CFDIS,CFL = CF(Polar_Code)
        C = 1/(2*(Lb/Breadth)**eb)
        Disp = max(Delta/1000, 10)

        if strip(upper(bow_type)) == 'SPOON':
            Kf = (2 * C * Breadth**(1-eb) / (1+eb))**0.9 * tan(radinas(psi_s))**(-0.9 * (1 + eb))
        elif strip(upper(bow_type)) == 'WEDGE':
            Kf = (tan(radians(alpha_s)) / tan(radians(psi_s))**2)**0.9
                
        Kh = 0.01*Awp
        Ki = Kf/Kh
        F1 = 0.534*Ki**0.15*sin(radians(psi_s))**0.2 * (Disp * Kh)**0.5 * CFL
        F2 = 1.2*CFF
        print round(Kh,3), round(Kf,3), round(Ki,3),round(F1,3), round(F2,3)
        return F1, F2, min(F1, F2)
