...
class ShipEngInput:
    ...
    def __init__(self, BookName, SheetName):
        ...
        if self.sociaty == "DNV":
            self.fps = sht.cell(13,2).value
            self.fb = sht.cell(14,2).value
            self.fR = sht.cell(15,2).value
            self.ffa = sht.cell(16,2).value
    
        # ffa : Fatigue coefficient to be taken as:, HCSR
class dnv_loads(ShipEngInput):
    ...
    def service_restrictions_reduction_factor(self, sn):
        fr = {'R0':1.0,'R1':0.9,'R2':0.8,'R3':0.7,'R4':0.6,'RE':0.5}
        return fr[sn]
    
    def Bilge_Keel_Factor(self):
        
        fBK = {'with Bilge Keel': 1.0, 'without Bilge Keel': 1.2 }
        return fBK[self.BK]
    
    def draught_ratio(self, TLC):
        TSC = self.Ts
        fT = (TLC/TSC)
...
# The roll angle, in deg, shall be taken as:
    def roll_angle(self):
        if self.Analysis_Type == "Strength":
            ...
            fp = self.fR * (0.23-4*self.fT*self.B * 1E-4)
          
        fBK = self.Bilge_Keel_Factor()
        self.theta = 9000 * (1.4 - 0.035 * self.Ttheta) * fp * fBK / ((1.15*self.B + 55)*pi)
        return self.theta
...
# 2.1.2 Pitch motion
    ...
    def pitch_angle(self):
        ...
        fT = 1.0
        fr = self.service_restrictions_reduction_factor(self.ServiceNotation)
        if self.Analysis_Type == "strength":
            fp = self.fps
        else:
            fp = fr * (0.27-0.02*fT)-(13-5*fT)*self.Ls * 1E-5
...
# The longitudinal acceleration due to surge, in m/s2, shall be taken as:
    def surge_acceleration_asurge(self):
        ...
        if self.Analysis_Type == "strength":
            ...
            fp = fr * (0.27-(15+4*fT)*self.Ls * 1E-5)
...
class hull_girder_strength(ShipEngInput):
